...
const FeatureMap = (
    {data, title, internal, spectrogram}:{data:number[][], title:string, internal?:boolean, spectrogram?:boolean} 
) => {
    if(!data || !data.length || !data[0]?.length) return null;
    const mapHeight = data.length;
    const mapWidth = data[0].length;

    const absMax = data.flat().reduce((acc, val) => {return Math.max(acc, Math.abs(val ?? 0), 0)})
